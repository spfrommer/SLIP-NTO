function [] = visualize( funparams, ntoParams, visParams )
    ...
    flightT = flightT + 0.00001; % hack to prevent duplicates if flightT is zero
    ...
    times = linspace(0, stanceT(1), ntoParams.gridn);

    i = ntoParams.gridn + 1;
    for p = 1 : size(ntoParams.phases, 1) - 1
        % Interpolate ballistic flight
        time = 0;
        nextStanceStart = transT(p * 2 + 1);
        
        while times(end) < nextStanceStart - visParams.dt
        ...
        %times = [times times(end)+visParams.dt];
        times = [times linspace(nextStanceStart, nextStanceStart + stanceT(p+1), ntoParams.gridn)];
        %times = times(1:end-1);
        i = i + ntoParams.gridn;
    end
